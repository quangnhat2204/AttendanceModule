...
def get_image():
    ...
    if request.method == 'GET':
        #url = request.args.get('url')
            ...
            filename = secure_filename(filename=label)
            ...
            if img is not None:
                ...
                if not os.path.exists(uploads_path + '/' + str(user)):
                    add_group_image(userId=user,uploads_path=uploads_path)
                else: pass
                save_image(img=img, filename=str(filename) + '.jpg' , uploads_path=uploads_path + '/' + str(user))
                ...
                if not os.path.exists(embeddings_path + '/' + str(user)):
                    add_group(userId=user,embeddings_path=embeddings_path)
                else: pass
                save_embedding(
                    embedding=embedding,
                    filename=filename,
                    embeddings_path=embeddings_path + '/' + str(user)
                )
    ...
    if request.method == 'POST':
        if 'file' not in request.files:
            ...
            # )
            result = "No 'file' field in POST request!"
        ...
        if file and allowed_file(filename=filename, allowed_set=allowed_set):
            ...
            if img is not None:
                ...
                #     status="Image uploaded and embedded successfully!"
                # )
                result = "Image uploaded and embedded successfully!"
                ...
                #     status="Image upload was unsuccessful! No human face was detected!"
                # )
                result = "Image upload was unsuccessful! No human face was detected!"
        ...
        # )
        result = "POST HTTP method required!"
    return result
    ...
    if request.method == 'GET':
        ...
        if img:
            for i in range(len(img)):
                ...
                embedding_dict = load_embeddings_byGroup(user)
                if embedding_dict:
                    # Compare euclidean distance between this embedding and the embeddings in 'embeddings/'
                    identity1 = identify_face(
                        embedding=embedding,
                        embedding_dict=embedding_dict,
                        user=user
                    )
                    identity.append(identity1)
                else:
                    # return render_template(
            ...
            if img:
            	for i in range(len(img)):
                 ...
                 if embedding_dict:
                    ...
                    #     identity="No embedding files detected! Please upload image files for embedding!"
                    # )
                    result = "No embedding files detected! Please upload image files for embedding!"

            	# return render_template(
             #        template_name_or_list='predict_result.html',
             #        identity=identity
             #        )
            	result = jsonify(identity)
...
@app.route("/deleteObject", methods=['GET'])
def delete_Object():
    if request.method == 'GET':
        name_object = request.args.get('id')
        user = request.args.get('user')
        filename = secure_filename(filename=name_object)
        delete_embedding(filename = str(filename) + '.npy',embeddings_path = embeddings_path + '/' + str(user))
        result = "Delete object successfully!"
    else:
        result = "GET HTTP method required!"
    return result

@app.route("/deleteAll", methods=['GET'])
def delete_Allobject():
    if request.method == 'GET':
        user = request.args.get('user')
        delete_all(embeddings_path = embeddings_path + '/' + str(user))
        result = "Delete all object successfully!"
    else:
        result = "GET HTTP method required!"
    return result
