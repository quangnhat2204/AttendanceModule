...
def get_face_predict(img, pnet, rnet, onet, image_size):
    minsize = 20
    threshold = [0.6, 0.7, 0.7]
    factor = 0.709
    margin = 44
...
def add_group_image(userId, uploads_path):
    group_path = uploads_path + '/' + str(userId)
    if os.path.exists(group_path): print("The file exists!")
    else:
        os.mkdir(group_path)

def add_group(userId, embeddings_path):
    group_path = embeddings_path + '/' + str(userId)
    if os.path.exists(group_path): print("The file exists!")
    else:
        os.mkdir(group_path)


def delete_embedding(filename, embeddings_path):
    file_embedding = embeddings_path + '/' + filename
    if os.path.exists(file_embedding):
        os.remove(file_embedding)
    else:
        print("The file does not exists")

def delete_all(embeddings_path):
    for embedding in glob.iglob(pathname=embeddings_path + '/*.npy'):
        os.remove(embedding)

def load_embeddings_byGroup(userId):    
    embedding_dict = defaultdict()

    for embedding in glob.iglob(pathname='embeddings/' + str(userId) + '/*.npy'):
...
def identify_face(embedding, embedding_dict, user):
    ...
    try:
        ...
        if min_distance <= 1.1:
            ...
            identity = identity[11+len(str(user))+1:]
